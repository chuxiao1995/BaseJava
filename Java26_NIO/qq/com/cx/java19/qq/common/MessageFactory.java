...
public class MessageFactory {
	...
	/**
	 * 组装客户端发送的私聊消息
	 * receiverUserInfo 消息接收者消息
	 * msg需要发送的信息
	 */
	public static byte[] popCientChatSingleMessage(String receiverUserInfo,String msg) {
		//接受者信息数组
		...
		//1消息类型
		System.arraycopy(Util.int2Bytes(Message.CLIENT_TO_SERVER_CHAT), 0, pack,0,4);		
	...
	public static byte[] popServerChatsMessage(String userInfo,byte[] clientChatMsg) {
		...
		//聊天内容长度
		System.arraycopy(Util.int2Bytes(clientChatMsg.length),0,allArr, (8 + userInfoArr.length), 4);
		//聊天内容
		System.arraycopy(clientChatMsg,0,allArr, (8 + userInfoArr.length + 4), clientChatMsg.length);
		return allArr;
	}
	/**
	 * 组装服务器端私聊消息
	 * @param userInfo
	 * @param ChatMsg: pure message
	 * @return
	 */
	public static byte[] popServerChatSingleMessage(String senderUserInfo,byte[] chatMsg) {
		...
		//1消息类型
		System.arraycopy(Util.int2Bytes(Message.SERVER_TO_CLIENT_CHAT_MESSAGE_SINGLE), 0, pack, 0, 4);
		//2目标用户信息长度
		System.arraycopy(Util.int2Bytes(senderInfo.length), 0, pack, 4, 4);
		//3目标用户信息内容
		System.arraycopy(senderInfo, 0, pack, 8, senderInfo.length);
		//4聊天内容长度
		System.arraycopy(Util.int2Bytes(chatMsg.length),0,pack, (8 + senderInfo.length), 4);
		//5聊天内容
		System.arraycopy(chatMsg,0,pack, (8 + senderInfo.length + 4), chatMsg.length);
		return pack;
	}
...
/************************************服务器解析部分**************************************/		
			...
			case Message.CLIENT_TO_SERVER_CHAT:	
				...
				is.read(chatBuf);
				//服务器端私聊
				return new ClientChatSingleMessage(new ServerSingleBean(recvInfo,chatBuf));
...
/***********************************客户端解析部分 **************************************/		
	 		...
	 		case Message.SERVER_TO_CLIENT_CHAT_MESSAGE_SINGLE:
	 			...
	 			//新建字符串数组包含用户信息和聊天内容
	 			return new ServerChatSingleMessage(new ClientSingleBean(strSenderUserInfo, strSenderChat));
